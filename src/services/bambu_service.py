...
;===== end move to safe position ====="""
    ...
    def send_print_file_direct(self, file_path: str) -> bool:
        """
        Send print file directly to printer SD card via FTPS (port 990)
        Then send MQTT start command
        
        This is the DIRECT method - no HTTP server needed!
        Based on OctoPrint-BambuPrinter plugin implementation
        
        Workflow:
        1. Upload file directly to printer via FTPS implicit SSL/TLS
        2. Send MQTT start command to print the uploaded file
        ...
        """
        import os
        from src.services.ftps_service import BambuDirectUploadService
        
        try:
            filename = os.path.basename(file_path)
            
            logger.info(f"🚀 DIRECT UPLOAD: Starting for {filename}")
            
            # Step 1: Upload file directly to printer via FTPS
            logger.info(f"Step 1: Uploading directly via FTPS to {self.printer_ip}...")
            
            upload_service = BambuDirectUploadService(self.printer_ip, self.access_code)
            
            # Upload with progress
            def progress_callback(sent: int, total: int):
                percent = (sent / total) * 100
                logger.info(f"Upload progress: {percent:.1f}% ({sent}/{total} bytes)")
            
            if not upload_service.upload_to_sd(file_path, filename, progress_callback):
                logger.error("Direct upload failed!")
                return False
            
            logger.info(f"✅ Direct upload complete: {filename}")
            
            # Step 2: Send start command via MQTT
            logger.info("Step 2: Sending start command via MQTT...")
            time.sleep(1)  # Wait 1 second for printer to register file
            ...
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                logger.info(f"✅ Print started for: {filename}")
                return True
            else:
